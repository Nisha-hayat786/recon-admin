...
const Vendors = () => {
    ...
    const filteredVendors = vendorData.filter(vendor =>
        ...
        vendor.accountId.toLowerCase().includes(search.toLowerCase()) ||
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-4 relative">
            ...
            <div className="flex items-center mb-6 gap-2">
                ...
                <button className='bg-blue-600 text-white px-4 py-2 rounded-lg' onClick={() => navigate('/admin/create-vendor')}>Create Vendor</button>
            </div>
                        ...
                        {filteredVendors.map((vendor, idx) => (
                            <tr key={vendor.id} className="hover:bg-gray-50">
                                <td className="px-4 py-3 flex items-center gap-3">
                                    <img src={vendor.logo} alt="logo" className="w-10 h-10 rounded-full border object-cover" />
                                    <div>
                                        <p className="font-medium text-gray-900">{vendor.name}</p>
                                        <p className="px-4 text-gray-700 text-xs">#{vendor.accountId}</p>
                                    </div>
                                </td>
                                {/* <td className="px-4 py-3 text-gray-700">{vendor.accountId}</td> */}
                                <td className="px-4 py-3">
                                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${statusStyles[vendor.status]}`}>{vendor.status}</span>
                                </td>
                                <td className="px-4 py-3 text-yellow-500 font-semibold flex items-center gap-1">
                                    {vendor.rating} <span className="text-yellow-400">★</span>
                                </td>
                                <td className="px-4 py-3">
                                    <div className="flex flex-col gap-1 text-xs">
                                        <span className="flex items-center gap-1"><FiMail className="inline w-4 h-4" /> {vendor.email}</span>
                                        <span className="flex items-center gap-1"><FiPhone className="inline w-4 h-4" /> {vendor.phone}</span>
                                        <span className="flex items-center gap-1"><FiMapPin className="inline w-4 h-4" /> {vendor.address}</span>
                                    </div>
                                </td>
                                <td className="px-4 py-3 text-gray-700">{vendor.timezone}</td>
                                <td className="px-4 py-3">
                                    <div className="flex">
