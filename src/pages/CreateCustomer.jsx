...
const CreateCustomer = () => {
  ...
  const addContact = () => {
    if (newContact.job && newContact.name && newContact.phone && newContact.email) {
      const contact = {
        id: Date.now(),
        ...newContact
      };
      setContacts([...contacts, contact]);
      setNewContact({ job: '', name: '', phone: '', email: '' });
  ...
  const addFile = () => {
    if (newFile.name && newFile.expires) {
      const file = {
        id: Date.now(),
        ...newFile
      };
      setFiles([...files, file]);
      setNewFile({ name: '', expires: '' });
  ...
  const handleNewFileChange = (field, value) => {
    setNewFile(prev => ({ ...prev, [field]: value }));
  };
      ...
      <form className="w-full">
        ...
        <div className="flex items-center gap-2 mt-4">
          <label className="flex items-center gap-2 cursor-pointer w-1/2">
            <p className='w-full font-semibold'>Contact Information</p>
        ...
        {showContact && (
          <>
              ...
              <div className="mt-4 p-4 border rounded-lg bg-gray-50">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <input
                    ...
                    onChange={(e) => handleNewContactChange('job', e.target.value)}
                  />
                  <input
                    className="input"
            ...
            {/* Contacts Table */}
            <div className="overflow-x-auto rounded-lg border border-gray-300 mt-2">
        ...
        {showPreQualify && (
          <div className="mt-2 border border-gray-300 rounded-lg p-4 bg-gray-50">
            <div className="mb-2 font-semibold">Accepted Badges:</div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              {Object.keys(badges).map(badge => (
                <label key={badge} className="flex items-center gap-2 cursor-pointer">
                  <p className='w-full font-semibold'>{badge}</p>
        ...
        {showFiles && (
          <>
            ...
            {showAddFileForm && (
              <div className="mt-4 p-4 border rounded-lg bg-gray-50">
                <h4 className="font-semibold mb-3">Add New File</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <input
                    className="input"
                    placeholder="File Name"
                    value={newFile.name}
                    onChange={(e) => handleNewFileChange('name', e.target.value)}
                  />
                  <input
                    className="input"
                    placeholder="Expires On"
                    type="date"
                    value={newFile.expires}
                    onChange={(e) => handleNewFileChange('expires', e.target.value)}
                  />
                </div>
                <div className="flex gap-2 mt-3">
                  <button
